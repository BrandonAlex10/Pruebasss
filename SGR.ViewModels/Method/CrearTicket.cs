...
namespace SGR.ViewModels.Method
{
    public class CrearTicket
    {
        ...
        int maxCar = 38, cortar;//Para una impresora ticketera que imprime a 38 columnas. La variable cortar cortara el texto cuando rebase el limte.
        ...
        //Creamos el primer metodo, este dibujara lineas guion.
        public string lineasSubGuion()
        {
            string lineasSubGuion = "";
            for (int i = 0; i < (maxCar/2); i++)
            {
                lineasSubGuion += "_ ";//Agregara un guio hasta llegar la numero maximo de caracteres.
            }
            return linea.AppendLine(lineasSubGuion.Trim()).ToString(); //Devolvemos la lineaSubGuion
        }
        ...
        //Creamos un metodo para poner el texto como comentario
        public void TextoComentario(string texto)
        {
            var ArraysComentario = texto.Split("-".ToArray());
            foreach (var item in ArraysComentario)
            {
                texto = "   --" + item.Trim();
                //Si la longitud del texto es mayor al numero maximo de caracteres permitidos, realizar el siguiente procedimiento.
                if (texto.Length > maxCar)
        ...
        //Metodo para agreagar texto Izquierda, centro y derecha
        public void TextoTriple(string TextoIzquierda, string TextoCentro, string TextoDerecha)
        {
            //Valida que cant precio e importe esten dentro del rango.
            if (TextoCentro.ToString().Length <= 5 && TextoDerecha.ToString().Length <= 7)
            {
                string elemento = "", espacios = "";
                int nroEspacios = 0;
                if (TextoIzquierda.Length >= 21)
                    TextoIzquierda.Substring(0, 21);
                //Si el nombre o descripcion del articulo es mayor a 20, bajar a la siguiente linea
                for (int i = 0; i < (21 - TextoIzquierda.Length); i++)
                {
                    espacios += " "; //Agrega espacios para completar los 20 caracteres
                }
                elemento = TextoIzquierda + espacios;

                if (TextoCentro.Length >= 6)
                    TextoCentro.Substring(0, 6);
                //Colocar la cantidad a la derecha.
                nroEspacios = (5 - TextoCentro.ToString().Length);
                espacios = "";
                for (int i = 0; i < nroEspacios; i++)
                {
                    espacios += " ";
                }
                elemento += TextoCentro.ToString() + espacios;

                if (TextoDerecha.Length >= 10)
                    TextoDerecha.Substring(0, 10);
                //Colocar el precio a la derecha.
                nroEspacios = (10 - TextoDerecha.ToString().Length);
                espacios = "";
                for (int i = 0; i <= nroEspacios; i++)
                {
                    espacios += " ";
                }
                elemento += espacios + TextoDerecha.ToString();
                linea.AppendLine(elemento);
            }
            else
            {
                linea.AppendLine("Los valores ingresados para esta fila");
                linea.AppendLine("superan las columnas soportdas por este.");
                throw new Exception("Los valores ingresados para algunas filas del ticket superan las columnas soportdas por éste.");
            }
        }
        ...
        public void TextoExtremos(string textoIzquierdo, string textoDerecho)
        {
            ...
            if (textoDerecho.Length > 18)//Si es mayor a 20 lo cortamos
            {
                cortar = textoDerecho.Length - 18;
                textoDer = textoDerecho.Remove(18, cortar);
            }
            else
        ...
        //Metodo para poner texto a los extremos aliniados
        public void TextoExtremosAlineados(string textoIzquierdo, string textoDerecho)
        {
            //variables que utilizaremos
            string textoIzq, textoDer, textoCompleto = "", espacios = "";

            //Si el texto que va a la izquierda es mayor a 18, cortamos el texto.
            if (textoIzquierdo.Length > 20)
            {
                cortar = textoIzquierdo.Length - 20;
                textoIzq = textoIzquierdo.Remove(20, cortar);
            }
            else
                textoIzq = textoIzquierdo;
            ...
            if (textoDerecho.Length > 16)//Si es mayor a 20 lo cortamos
            {
                cortar = textoDerecho.Length - 16;
                textoDer = textoDerecho.Remove(16, cortar);
            }
            else
            { textoDer = textoDerecho; }

            //Obtenemos el numero de espacios restantes para poner textoDerecho al final
            int nroEspacios = (maxCar/2) + 2 - textoIzq.Length;
            for (int i = 0; i < nroEspacios; i++)
            {
                espacios += " ";//agrega los espacios para poner textoDerecho al final
            }
        ...
        public void AgregarTotales(string texto, decimal total)
        {
            ...
            if (texto.Length > 18)//Si es mayor a 25 lo cortamos
            {
                cortar = texto.Length - 18;
                resumen = texto.Remove(18, cortar);
            }
            else
                resumen = texto;
                ...
                if (articulo.Length > 18)
                {
                    ...
                    for (int longitudTexto = articulo.Length; longitudTexto > 18; longitudTexto -= 18)
                    {
                        if (bandera == false)//si es false o la primera linea en recorrerer, continuar...
                        {
                            ...
                            linea.AppendLine(articulo.Substring(caracterActual, 18));//Solo agrega el nombre del articulo
        ...
        public void ImprimirTicketCopia(string impresora)
        {
            //Cortamos Ticket
            linea.AppendLine("\x1B" + "d" + "\x03");
            linea.AppendLine("\x1B" + "i" + "\x11" + "\xa" + "\x01");

            var list = new List<object>();
            foreach (String strPrinter in System.Drawing.Printing.PrinterSettings.InstalledPrinters)
                list.Add(strPrinter);

            var ArrayImpresora = impresora.Split('\\');

            var ImpresoraImprimir = list.FirstOrDefault(x => x.ToString() == impresora || x.ToString() == (ArrayImpresora[ArrayImpresora.Length - 1].ToUpper()));

            if (ImpresoraImprimir == null)
                throw new Exception("Impresora '" + impresora + "' no instalada en este equipo.");
            else
                RawPrinterHelper.SendStringToPrinter(ImpresoraImprimir.ToString(), linea.ToString()); //Imprime texto.
        }
        ...
        {
            //Cortamos Ticket
            //linea.AppendLine("\x1B" + "P" + "\x05"); //Avanza 5 renglones
            linea.Replace("\x1B" + "d" + "\x03", "");
            linea.AppendLine("\x1B" + "d" + "\x03");
            linea.Replace("\x1B" + "i" + "\x11" + "\xa" + "\x01", "");
            linea.AppendLine("\x1B" + "i" + "\x11" + "\xa" + "\x01");
            ...
            if (ImpresoraImprimir == null)
                ...
                RawPrinterHelper.SendStringToPrinter(ImpresoraImprimir.ToString().Trim(), linea.ToString()); //Imprime texto.
    ...
    public class RawPrinterHelper
    {
        ...
        public static bool SendBytesToPrinter(string szPrinterName, IntPtr pBytes, Int32 dwCount)
        {
            ...
            di.pDocName = "SGR_Impresion";//Este es el nombre con el que guarda el archivo en caso de no imprimir a la impresora fisica.
